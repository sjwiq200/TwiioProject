...
<!-- ---------Floating Button------------ -->
<link href="/resources/css/floatingButton.css" rel="stylesheet"
	type="text/css" />
...
.mainBackImage {
	background-image:
		url("/resources/images/dailyPlanContent/daum_gallery_photo_20160913155706.jpg");
	opacity: 0.3;
}
...
<script>
	...
	$(function() {
		$(".select").bind(
				...
				function() {
					...
					var mainPlanNo = $($("input[name='mainPlanNo']")[index])
							.val();
					var url = "/dailyplan/getDailyPlanFromMain?mainPlanNo="
							+ mainPlanNo;
					$(location).attr('href', url);
				});
	})
	...
	$(function() {
		...
		$("input[name='shareButton']")
				.bind(
						"click",
						function() {
							var index = $("input[name='shareButton']").index(
									this);
							var mainPlanNo = $(
									$("input[name='mainPlanNo']")[index]).val();
							...
							$
									.ajax({
										url : "/dailyplan/json/listFriend?mainPlanNo="
												+ mainPlanNo,
										method : "GET",
										dataType : "json",
										headers : {
										...
										success : function(JSONData) {
											...
											for (var i = 0; i < friend.length; i++) {
												...
												result += '<div class="col-sm-3 col-sm-offset-1" ><img src="/resources/images/userimages/'+friend[i].userImage+'" style="width: 50px; height: 50px;" class="media-photo"></div>'
														...
														+ friend[i].userName
														+ '</div>'
														+ '<input type="button" id="sharePlan'
														+ i
														+ '" class="btn-dy1" onclick="sharePlan('
														+ friend[i].userNo
														+ ','
														+ i
														+ ','
	...
	function sharePlan(userNo, i, mainPlanNo) {
		$.ajax({
			url : '/dailyplan/json/sharePlan?userNo=' + userNo + "&mainPlanNo="
					+ mainPlanNo,
			type : 'get'
		});
		swal("트위북이 공유되었습니다.");
	...
	$(function() {
		$("#submit").on(
				...
				function() {
					...
					if (($("#mainThumbnail").val() != "")
							&& ($("#datepicker2").val() != "")
							&& ($("#datepicker1").val() != "")
							&& ($("#planTitle").val() != "")
							&& ($("#country1").val() != "")) {
						$("form[name='addMainForm']").attr("method", "POST")
								.attr("action", "/mainplan/addMainPlan").attr(
		...
		$("#addCountry")
				.on(
						...
						function() {
							...
							$("div[name=addCountry]")
									.append(
											$('<input  type="text" id="country'+i+'" name="countryList" style="position: absoloute" class="form-control input-md contents" >'));
							$(document)
									.find("input[name='countryList']")
	...
	function readURL(input) {
		if (input.files && input.files[0]) {
			...
			reader.onload = function(e) {
				$('#blah').attr('src', e.target.result).attr('width', '100px')
						.attr("style", "border-radius : 25px").attr('height',
								'100px');
				$('#photoButton').remove();
			}
...
<body>
	...
	<form>
		...
		<div class="container">
			...
			<p
				style="font-family: TYPO_JEONGJOL; font-size: 1.5em; margin-bottom: 70px;"
				align="center">여행을 맞이하는, 설렘부터</p>
			</header>
									...
									<div id="thumbnail">
										<div class="caption">
											...
											<div class="row">
												...
												<div class="col-md-4 thumbnailClass"
												...
												<div class="col-md-6 textArea">
													<div class="col-md-10 select">
														...
														<div style="font-family: 'JEJUGOTHIC'">
															<p>${mainPlan.departureDate } ~
																${mainPlan.arrivalDate}</p>
															<p>
																<Strong>나라</Strong> ${mainPlan.country}
															</p>
															<p>
																<Strong>도시</Strong> ${mainPlan.city}
															</p>
														</div>
													</div>
													<div class="col-md-2" align="center"
														style="margin-top: 35px; font-family: 'JEJUGOTHIC'">
														<input type="button" class="btn-dy2" name="shareButton"
															id="shareButton" value="공유"
															style="margin-bottom: 5px; font-family: \'JEJUGOTHIC\';" />
														<input type="button" class="btn-dy2" name="updateButton"
															id="updateButton" value="수정"
															style="margin-bottom: 5px; font-family: \'JEJUGOTHIC\';" />
														<input type="button" class="btn-dy2" name="deleteButton"
															id="deleteButton" value="삭제"
															style="margin-bottom: 5px; font-family: \'JEJUGOTHIC\';" />

													</div>
												</div>

												<div class="col-md-1" id="buttonBox" align="right"></div>
											</div>
											<!--  row 끝 -->
	...
	<div class="modal-dialog modal-md">
		...
		<div class="modal-content">
			<div class="modal-header"
				style="background-image: url('/resources/images/dailyPlanContent/friendsShare.jpg');">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title" align="center"
					style="font-family: 'TYPO_JEONGJOL'; margin-top: 50px; margin-bottom: 50px; color: #ffffff;">
					<Strong>나의 플랜 북을 친구들과 공유할까요?</Strong>
				</h4>
			</div>
			<div class="modal-body col-sm-12" align="center"
...
<div id="addMainModal" class="modal fade" role="dialog"
	...
	<div class="modal-dialog" align="center"
		...
		<form name="addMainForm">
			...
			<div class="col-sm-12 form-group center-block contentsList"
 ...
 */ background-color: #ffffff; border-radius: 3% !important; border: 1px dashed #3B3B3B; color: #3B3B3B !important;">
				<div
					style="font-size: 1.5em; font-family: Pacifico; margin-top: 50px; margin-bottom: 20px; color: #D73A31; opacity: 0.8;">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
				...
				<div
					style="font-size: 1.5em; margin-bottom: 50px; font-family: TYPO_JEONGJOL; color: #3B3B3B; opacity: 0.8;">당신의
					트위북을 만들어보세요</div>
				<div class="col-sm-2"></div>
				<div class="col-sm-8">
					<div name="addThumbnail">
						<label for="mainThumbnail" class="col-sm-12 control-label">표지</label>
						<div id="fileUpload" name="fileUpload">
							<img id="blah" /> <img
								src="/resources/images/icon/plan/imageInput.png"
								id="photoButton" width="20px" style="position: absoloute">
						</div>
 ...
 -->
					...
					<input type="file" class="col-sm-offset-3 form-control contents"
						id="mainThumbnail" name="file" style="display: none;">


					<div name="planer">
						<label for="planTitle" class="col-md-12 control-label">제목</label>
						<input type="text" class="form-control contents"
							style="position: absoloute" id="planTitle" name="planTitle"
							placeholder="Your book title">
						<p>&nbsp;</p>

						<div name="addCountry">
							<label class="control-label" for="textinput">국가</label> <a
								class="btn btn-default btn" href="#" role="button"
								id="addCountry" name="addCountry"> <span
								class="glyphicon glyphicon-plus" aria-hidden="true"
								style="color: #A6A0A5;"></span></a> <a class="btn btn-default btn"
								href="#" role="button" id="removeCountry" name="removeCountry"
								disabled="true"> <span class="glyphicon glyphicon-minus"
								aria-hidden="true" style="color: #A6A0A5;"></span></a> <input
								type="text" id="country1" name="countryList"
						...
						<input type="text" class="form-control contents" id="datepicker1"
							name="departureDate" placeholder="Your departure date">
						<p>&nbsp;</p>
						...
						<div style="margin-bottom: 50px;">
							<a class="btn-dy" style="margin-bottom: 50px;" id="submit">여행을
								시작할까요?</a>
						</div>
