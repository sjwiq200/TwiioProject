...
<script
	src="//cdnjs.cloudflare.com/ajax/libs/jspdf/0.9.0rc1/jspdf.min.js"></script>
<script
		...
		$(function() {
			$("div[name=contentsBox]").hover(function() {
				var index = $("div[name=contentsBox]").index(this);
 				$($("div[name=contentsBox]")[index]).attr("style","background-color:#A6A0A5; color:white; opacity:0.5;");
 					}, function(){
 						var index = $("div[name=contentsBox]").index(this);
				...
				<div class="col-xs-12 inner" id="thumbnailMainThumbBox"
					...
					<!-- -------------TOP<END>--------------- -->
						...
						<div class="PlanContentList" id="PlanContentList" align="center"
							style="font-family: 'JEJUGOTHIC' !important; font-size: 1.1em !important; font-color: #C2C2C2 !important;">
							...
							<c:forEach var="planContent" items="${list}" varStatus="index">
								...
								<c:if test="${!empty scrap.scrapNo }">
									...
									<c:if test="${!empty planContent.routeDescription}">
										<p class="contents"
											...
											<!-- 길찾기결과 :  -->${planContent.routeDescription}</p>
									</c:if>
									...
									<c:if test="${!empty planContent.mapImage}">
										<div class="col-xs-12" style="margin-top: 20px" align="center">
											<p class="contents">
												<Strong> <img
													src="/resources/images/dailyPlanContent/map.png"
													width="40px">
												</Strong>
											</p>
										</div>
										<div class="col-xs-6" align="center">
											<p class="contents">
												<img
													src="${planContent.mapImage}&key=AIzaSyCmTcIdw0uowsiJrs4YNA0lhjLnN8PigjE"
													name="mapImg" class="contentsDelete" width="300px"
									...
									<c:if test="${!empty planContent.mapName}">
										<div class="col-xs-6" style="margin-top: 30px;" align="left">
											<p>&nbsp;</p>
											<p class="contents"
												style="font-size: 1.1em !important; font-color: #C2C2C2 !important;">[${planContent.mapName}]</p>
									</c:if>

									<c:if test="${!empty planContent.mapAddress}">
										<p class="contents"
											style="font-size: 0.9em !important; font-color: #C2C2C2 !important;">ADDRESS
											: ${planContent.mapAddress}</p>
									</c:if>

									<c:if test="${!empty planContent.mapPhone}">
										<p class="contents"
											style="font-size: 0.9em !important; font-color: #C2C2C2 !important;">PHONE
											: ${planContent.mapPhone}</p>
									</c:if>

									<c:if test="${!empty planContent.mapWebsite}">
										<p class="contents"
											style="font-size: 0.9em !important; font-color: #C2C2C2 !important;">WEBSITE:
								...
								<c:if test="${!empty planContent.contentText}">
									...
									<div class="col-xs-12" align="center"
										style="margin-top: 20px; font-size: 1.1em !important; font-color: #C2C2C2 !important;">
								...
								<c:if test="${!empty planContent.contentImage}">
									<p class="contentsThis">
										<img
											...
											class="contentsDelete" width="400px"
					...
					<form name="form" enctype="multipart/form-data">
						<div class="form-group">
							...
							<div class="filebox bs3-primary preview-image">
								<input class="upload-name col-sm-4 col-sm-offset-3" value="파일선택"
									disabled="disabled" style="width: 200px;"> <label
									for="input_file" class="col-sm-2 col-offset-4">사진 선택</label> <input
									type="file" id="input_file" name="uploadFile"
									class="upload-hidden">
							</div>
						</div>
						<br /> <input type="hidden" name="dailyPlanNo"
							value="${dailyPlan.dailyPlanNo}" /> <input type="hidden"
							name="mainPlanNo" value="${dailyPlan.mainPlan.mainPlanNo}" />
						<button name="add"
							class="btn btn-primary btn-sm col-sm-2 col-sm-offset-5 text-center"
